...
class ExtractTranslation extends Command
{
     ...
     * @var string
     */
    protected $description = 'Will perform various operation regarding translation for WickxPOS and it\'s modules';
    ...
    private function extractForModuleLanguage( $lang, $module, $files )
    {
        ...
        $this->newLine();
        $this->info( sprintf( __( 'Localization for %s extracted to %s' ), config( 'wickxpos.languages' )[ $lang ], $filePath ) );
    }
        ...
        if ( $this->argument( 'module' ) ) {
            ...
            if ( ! empty( $module ) ) {
                ...
                if ( $this->option( 'lang' ) === 'all' ) {
                    foreach ( config( 'wickxpos.languages' ) as $lang => $humanName ) {
    ...
    private function extractLanguageForSystem( $lang, $files )
    {
        ...
        $this->newLine();
        $this->info( 'Extraction complete for language : ' . config( 'wickxpos.languages' )[ $lang ] );
    }
