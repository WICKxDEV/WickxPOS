...
class SetupCommand extends Command
{
     ...
     * @var string
     */
    protected $description = 'Install WickxPOS from the command line.';
    ...
    public function handle()
    {
        ...
        if ( Helper::installed() ) {
            return $this->error( __( 'WickxPOS is already installed.' ) );
        }
        ...
        if ( in_array( strtolower( $answer ), [ 'y', 'yes' ] ) || $this->requireConfirmation === false ) {
            ...
            return $this->info( 'Thank you, WickxPOS has been successfully installed.' );
        } else {
    ...
    private function setupLanguage()
    {
        while ( empty( $this->language ) ) {
            $langIndex = $this->choice( __( 'In which language would you like to install WickxPOS ?' ), array_values( config( 'wickxpos.languages' ) ) );
            $this->language = array_keys( config( 'wickxpos.languages' ) )[ $langIndex ];
