...
class ReportService
{
    ...
    public function handleStockAdjustment( ProductHistory $history )
    {
        if ( in_array( $history->operation_type, [
            ...
            $message = __( 'A stock operation has recently been detected, however WickxPOS was\'nt able to update the report accordingly. This occurs if the daily dashboard reference has\'nt been created.' );
            ...
            $notification->create(
                ...
                description: $message,
                url: 'https://my.wickxpos.com/en/troubleshooting/untracked-stock-operation',
            )->dispatchForGroup(
    ...
    {
        $orderProductTable = Hook::filter( 'ns-model-table', 'wickxpos_orders_products' );
        $orderTable = Hook::filter( 'ns-model-table', 'wickxpos_orders' );
    ...
    public function getCombinedProductHistory( $date, $categories, $units )
    {
        $request = DB::query()->select( [
            ...
            'wickxpos_products.category_id as product_category_id',
            'wickxpos_units.name as unit_name',
            'wickxpos_products_histories_combined.date as history_date',
            'wickxpos_products_histories_combined.initial_quantity as history_initial_quantity',
            'wickxpos_products_histories_combined.procured_quantity as history_procured_quantity',
            'wickxpos_products_histories_combined.defective_quantity as history_defective_quantity',
            'wickxpos_products_histories_combined.sold_quantity as history_sold_quantity',
            'wickxpos_products_histories_combined.final_quantity as history_final_quantity',
            'wickxpos_products_histories_combined.unit_id as history_unit_id',
            'wickxpos_products_histories_combined.product_id as history_product_id',
            'wickxpos_products_histories_combined.name as history_name',
        ] )->from( 'wickxpos_products_unit_quantities' )
            ->rightJoin( 'wickxpos_products', 'wickxpos_products.id', '=', 'wickxpos_products_unit_quantities.product_id' )
            ->rightJoin( 'wickxpos_products_histories_combined', function ( $join ) use ( $date ) {
                $join->on( 'wickxpos_products_histories_combined.product_id', '=', 'wickxpos_products_unit_quantities.product_id' );
                $join->on( 'wickxpos_products_histories_combined.unit_id', '=', 'wickxpos_products_unit_quantities.unit_id' );
                $join->where( 'wickxpos_products_histories_combined.date', $date );
            } )
            ->rightJoin( 'wickxpos_units', 'wickxpos_units.id', '=', 'wickxpos_products_histories_combined.unit_id' );

        /**
         * Will only pull products that belongs to the units id provided.
         */
        if ( ! empty( $units ) ) {
            $request->whereIn( 'wickxpos_products_histories_combined.unit_id', $units );
        }

        if ( ! empty( $categories ) ) {
            $request->whereIn( 'wickxpos_products.category_id', $categories );
        }

        $request->where( 'wickxpos_products_histories_combined.date', Carbon::parse( $date )->format( 'Y-m-d' ) );
