...
class OrdersService
{
    ...
    private function __checkOrderPayments( $fields, ?Order $order, Customer $customer )
    {
         ...
         */
        if ( $totalPayments > 0 ) {
            ns()->restrict( 'wickxpos.make-payment.orders', __( 'You\'re not allowed to make payments.' ) );
        }
    ...
    public function generateOrderCode( $order )
    {
        ...
        $count = DB::table( Hook::filter( 'ns-model-table', 'wickxpos_orders_count' ) )
            ->where( 'date', $today )
            ->value( 'count' );

        if ( $count === null ) {
            $count = 1;
            DB::table( Hook::filter( 'ns-model-table', 'wickxpos_orders_count' ) )
                ->insert( [
                    'date' => $today,
                    'count' => $count,
                ] );
    ...
    {
        $payments = config( 'wickxpos.orders.statuses' );
    ...
    {
        return config( 'wickxpos.orders.statuses' );
    }

    public function getRefundedOrderProductLabel( $label )
    {
        return config( 'wickxpos.orders.products.refunds' )[ $label ] ?? __( 'Unknown Product Status' );
    }
    ...
    public function notifyExpiredLaidAway()
    {
        ...
        if ( ! $orders->isEmpty() ) {
            ...
            $notificationService->create(
                ...
                Role::namespace( 'wickxpos.store.administrator' ),
            ] );
