...
class CashRegistersService
{
    ...
    public function getZReport( Register $register )
    {
        ...
        $payments = OrderPayment::whereIn( 'order_id', $orders->pluck( 'id' ) )
            ->select( 'wickxpos_payments_types.identifier', DB::raw( 'SUM(value) as total_amount' ), 'label' )
            ->groupBy( [ 'identifier', 'label' ] )
            ->join( 'wickxpos_payments_types', 'wickxpos_payments_types.identifier', '=', 'wickxpos_orders_payments.identifier' )
            ->get();
