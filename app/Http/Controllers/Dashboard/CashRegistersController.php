    ...
    public function getSessionHistory( Register $register )
    {
        if ( $register->status === Register::STATUS_OPENED ) {
            $lastOpening = $register->history()
                ...
                ->orderBy( Hook::filter( 'ns-table-name', 'wickxpos_registers_history' ) . '.id', 'desc' )
                ->first();
            ...
            if ( $lastOpening instanceof RegisterHistory ) {
                ...
                $historyRequest = $register->history()
                    ->select( [
                        ...
                        'wickxpos_registers_history.description as description',
                        'wickxpos_payments_types.label as payment_label',
                    ] )
                    ->with( 'order' )
                    ->leftJoin( 'wickxpos_payments_types', 'wickxpos_registers_history.payment_type_id', '=', 'wickxpos_payments_types.id' )
                    ->where( 'wickxpos_registers_history.id', '>=', $lastOpening->id );
                ...
                $totalPaymentTypeSummary = $historyRequest
                    ...
                    ->select( [
                        ...
                        'wickxpos_payments_types.label as label',
                        'action',
                        'wickxpos_registers_history.description as description',
                    ] )
                    ->groupBy( [ 'action', 'wickxpos_payments_types.label', 'description' ] )
                    ->get()
                    ->map( function ( $group ) {
                        $color = 'info';
