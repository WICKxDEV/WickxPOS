...
class CustomerCrud extends CrudService
{
     ...
     */
    protected $table = 'wickxpos_users';
    ...
    public $relations = [
        'leftJoin' => [
            [ 'wickxpos_customers_groups as group', 'wickxpos_users.group_id', '=', 'group.id' ],
        ],
        [ 'wickxpos_users as user', 'user.id', '=', 'wickxpos_users.author' ],
    ];
    ...
    protected $permissions = [
        'create' => 'wickxpos.create.customers',
        'read' => 'wickxpos.read.customers',
        'update' => 'wickxpos.update.customers',
        'delete' => 'wickxpos.delete.customers',
    ];
    ...
    {
        $query->join( 'wickxpos_users_roles_relations', 'wickxpos_users.id', '=', 'wickxpos_users_roles_relations.user_id' );
        $query->join( 'wickxpos_roles', 'wickxpos_roles.id', '=', 'wickxpos_users_roles_relations.role_id' );
        $query->where( 'wickxpos_roles.namespace', Role::STORECUSTOMER );
                        ...
                        FormInput::text(
                            ...
                            validation: collect( [
                                ns()->option->get( 'ns_customers_force_unique_phone', 'no' ) === 'yes' ? (
                                    $entry instanceof Customer && ! empty( $entry->phone ) ? Rule::unique( 'wickxpos_users', 'phone' )->ignore( $entry->id ) : Rule::unique( 'wickxpos_users', 'phone' )
                                ) : '',
                            ] )->toArray(),
    ...
    public function filterPostInputs( $inputs )
    {
        ...
        if ( empty( $inputs[ 'email' ] ) ) {
            ...
            $lastCustomer = User::orderBy( 'wickxpos_users.id', 'desc' )->first();
            ...
            $inputs[ 'email' ] = 'customer-' . $lastCustomerId + 1 . '@' . ( $domain[ 'host' ] ?? 'wickxpos.com' );
        }
