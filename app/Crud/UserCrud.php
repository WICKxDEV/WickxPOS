...
class UserCrud extends CrudService
{
    ...
    public $relations = [
        'leftJoin' => [
            ...
            [ 'wickxpos_users as author', 'wickxpos_users.author', '=', 'author.id' ],
        ],
    ];
    ...
    public function getForm( $entry = null )
    {
        return [
            'main' => [
                ...
                'validation' => $entry === null ? 'required|unique:wickxpos_users,username' : [
                    'required',
                    Rule::unique( 'wickxpos_users', 'username' )->ignore( $entry->id ),
                ],
                ...
                'general' => [
                    ...
                    'fields' => [
                        [
                            ...
                            'name' => 'email',
                            'label' => __( 'Email' ),
                            'validation' => $entry === null ? 'required|email|unique:wickxpos_users,email' : [
                                'required',
                                'email',
                                Rule::unique( 'wickxpos_users', 'email' )->ignore( $entry->id ),
                            ],
                            ...
                            'validation' => collect( [
                                ns()->option->get( 'ns_customers_force_unique_phone', 'no' ) === 'yes' ? (
                                    $entry instanceof User && ! empty( $entry->phone ) ? Rule::unique( 'wickxpos_users', 'phone' )->ignore( $entry->id ) : Rule::unique( 'wickxpos_users', 'phone' )
                                ) : '',
                            ] )->toArray(),
