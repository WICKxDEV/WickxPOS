...
class SetupAppTest extends TestCase
{
    ...
    public function test_configure_app()
    {
        ...
        $setupService->runMigration( [
            ...
            'password' => '123456',
            'admin_email' => 'contact@wickxpos.com',
            'store_name' => 'WickxPOS',
        ] );
         ...
         * we need to test if the store name is "WickxPOS"
         */
        $user = User::where( 'email', 'contact@wickxpos.com' )->first();
        $options = new Options;

        $this->assertTrue( $user instanceof User );
        $this->assertTrue( $options->get( 'ns_store_name' ) === 'WickxPOS', 'The store name wasn\'t loaded on the app options' );
